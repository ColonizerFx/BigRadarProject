                ...
                <div class="border-b border-gray-50 last:border-0">
                    <div class="px-6 py-4 flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            @if(count($parts) > 0)
                                <select
                                    @change="selectPart('{{ $slotLabel }}', $event.target.value, $event.target.options[$event.target.selectedIndex].dataset.price, $event.target.options[$event.target.selectedIndex].dataset.name, $event.target.options[$event.target.selectedIndex].dataset.availability)"
                                ...
                                <div x-show="selectedParts['{{ $slotLabel }}']" class="mt-2 flex items-center gap-2 text-xs text-gray-500">
                                    <span class="w-2 h-2 rounded-full"
                                          :class="selectedParts['{{ $slotLabel }}']?.availability === 'In Stock' ? 'bg-brand' : 'bg-yellow-400'"></span>
        ...
        <div class="w-full lg:w-80 flex-shrink-0 sticky top-6 h-max">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-900 overflow-hidden">
                ...
                <div class="px-6 py-4 space-y-3 max-h-72 overflow-y-auto">
                    ...
                    <div x-show="selectedParts['{{ $slotLabel }}']" class="flex justify-between items-start gap-2 text-sm">
                        <div>
                            ...
                            <div class="text-gray-900 font-medium text-xs mt-0.5 leading-tight" x-text="selectedParts['{{ $slotLabel }}']?.name ?? ''"></div>
                        </div>
                ...
                <div class="px-6 py-5 border-t border-gray-100 bg-gray-50">
                    ...
                    {{-- CTA --}}
                    <a href="{{ url('/products') }}" class="block w-full text-center bg-brand hover:bg-brand-dark text-white font-bold py-3.5 rounded-full hover-lift transition-all text-sm shadow-md border border-gray-900">
                        🛒 Shop These Parts
                    </a>
                    <a href="{{ url('/marketplace') }}" class="block w-full text-center bg-white border border-gray-900 text-gray-900 hover:bg-gray-50 font-bold py-3 rounded-full transition-all text-sm mt-3">
                        Browse Used Marketplace
                    </a>
                </div>
            </div>
...
<script>
    function pcBuilder() {
        return {
            ...
            selectPart(slot, id, price, name, availability) {
                if (!id) {
                    delete this.selectedParts[slot];
                    this.selectedParts = { ...this.selectedParts };
                    return;
                }
                this.selectedParts[slot] = { id, price: parseFloat(price) || 0, name, availability };
                this.selectedParts = { ...this.selectedParts };
            },
            clearAll() {
                this.selectedParts = {};
